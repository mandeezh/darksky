...
class PayloadSdkInterface:
    ...
    def sdkInitConnection(self) -> bool:
        if self.connection_type == CONTROL_UDP:
            ...
            try:
                ...
                # == Darksky start ====
                # T7 starts sending telemetry only after it first receives a heartbeat from the Jetson.
                # Wake up T7 MAVLink output
    ...
    def checkPayloadConnection(self, timeout: float = None) -> bool:
        ...
        while not self.time_to_exit:
            ...
            # commented by darksky to avoid port are simultaneously visited by two threads 
            # try:
            ...
            # use self.msg and msg=self.msg by darksky
            if self.msg is None:
                continue
            msg = self.msg 
            # ---- end of darksky change ---  
            ...
            if result:
                ...
                print("system is: ", sys_id, "gimbal is: ",comp_id)
                return True
   ...
   # Request the value of a specific camera parameter by ID.
    ...
    def payload_recv_handle(self) -> None:
        ...
        while self.time_to_exit == False:
            ...
            # ==== darksky changed it: use self.msg, so when recv_match called, the msg will be saved and can be used by other function , to avoid the ports be visited simultaneously by two thread
            self.msg = self.master.recv_match(blocking=True, timeout=config.communication.MESSAGE_TIMEOUT)
